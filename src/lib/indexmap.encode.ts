/**
 * indexmap::IndexMap / IndexSet encoders (rkyv's `indexmap-2` feature).
 *
 * See `./indexmap.decode.ts` for the `ArchivedIndexMap` byte layout.
 *
 * Serialization order (matching rkyv exactly): table buckets (reverse slot
 * order) → control bytes → per-entry dependencies in insertion order →
 * aligned entries array. The entries pointer is always a real pointer (even
 * when empty); only the table pointer uses the invalid sentinel for empty
 * maps.
 */
...
export class IndexMapEncoder<K, V> extends BaseEncoder<Map<K, V>, IndexResolver, IndexLayout> {
  #key: Encoder<K>;
  #value: Encoder<V>;
  ...
  constructor(
    ...
    requireHashableKey(keyCodec, 'indexMap');
    this.#key = keyCodec;
...
/**
 * indexmap::IndexMap<K, V> — insertion-order preserving hash map (write
 * half).
 */
export function indexMap<K extends AnyEncoder, V extends AnyEncoder>(
  keyCodec: K,
  valueCodec: V,
  options?: HashTableOptions,
): Encoder<Map<Infer<K>, Infer<V>>> {
  return new IndexMapEncoder(keyCodec, valueCodec, options?.hasher);
}

/**
 * indexmap::IndexSet<T> — a thin wrapper over `IndexMap<T, ()>`.
 */
export function indexSet<E extends AnyEncoder>(
  element: E,
  options?: HashTableOptions,
): Encoder<Set<Infer<E>>> {
  return new SetOfMapEncoder(new IndexMapEncoder<Infer<E>, null>(element, unit, options?.hasher));
}
